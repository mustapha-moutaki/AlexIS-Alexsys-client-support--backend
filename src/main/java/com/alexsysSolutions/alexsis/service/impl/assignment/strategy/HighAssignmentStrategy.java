...
@Component
public class HighAssignmentStrategy implements AssignmentStrategy {
    ...
    public Agent assign(List<Agent> agents) {
        ...
        return agents.stream()
                .sorted(Comparator
                        .comparing(Agent::getActiveTicketsCount)
                        .thenComparing(Agent::getPerformanceRating).reversed())
                .findFirst()
                .orElseThrow(() -> new ResourceNotFoundException("No available agent for HIGH priority"));
    }
}
