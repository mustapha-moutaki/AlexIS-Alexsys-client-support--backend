...
public class CategoryController {
    ...
    public ResponseEntity<ApiResponse<CategoryDtoResponse>> create(
        ...
        logger.info("st: POST /api/v1/categories - Creating category with name: {}", dto.getName());
        try {
            ...
            logger.info("st: Category created successfully with ID: {}", category.getId());
            return ResponseEntity.status(HttpStatus.CREATED).body(response);
        } catch (Exception e) {
            logger.error("st: Error creating category", e);
            throw e;
        }
    }

    // st: Update an existing category
    @PatchMapping("/{id}")
    public ResponseEntity<ApiResponse<CategoryDtoResponse>> update(
        ...
        logger.info("st: PATCH /api/v1/categories/{} - Updating category", id);
        try {
            ...
            logger.info("st: Category with ID {} updated successfully", id);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("st: Error updating category with id: {}", id, e);
            throw e;
        }
    }

    // st: Get category by id
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<CategoryDtoResponse>> getById(
            @PathVariable Long id,
            HttpServletRequest request
    ) {
        logger.info("st: GET /api/v1/categories/{} - Fetching category by id", id);
        try {
            ...
            response.setPath(request.getRequestURI());
            response.setStatus(HttpStatus.OK.value());
            logger.info("st: Category with ID {} retrieved successfully", id);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("st: Error fetching category with id: {}", id, e);
            throw e;
        }
    }

    // st: Get all categories with pagination and search
    @GetMapping
    public ResponseEntity<ApiResponse<Page<CategoryDtoResponse>>> getAll(
        ...
        logger.info("st: GET /api/v1/categories - Fetching all categories with page: {}, size: {}, searchKeyword: {}",
        ...
        try {
            ...
            response.setPath(request.getRequestURI());
            response.setStatus(HttpStatus.OK.value());
            logger.info("st: Retrieved {} total categories", categories.getTotalElements());
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("st: Error fetching categories", e);
            throw e;
        }
    }

    // st: Delete a category
    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable Long id,
            HttpServletRequest request
    ) {
        logger.info("st: DELETE /api/v1/categories/{} - Deleting category", id);
        try {
            categoryService.delete(id);
            ApiResponse<Void> response = ApiResponse.success("Category deleted successfully", null);
            response.setPath(request.getRequestURI());
            response.setStatus(HttpStatus.OK.value());
            logger.info("st: Category with ID {} deleted successfully", id);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("st: Error deleting category with id: {}", id, e);
            throw e;
        }
    }
}
